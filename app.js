...
const partLabel=label('ÂNODO',new THREE.Vector3(-.76,-2.02,.25),0xc8ff32);
...
const selectionBox=new THREE.BoxHelper(layers.anode,0xc8ff32);selectionBox.material.transparent=true;selectionBox.material.opacity=.7;scene.add(selectionBox);
...
function animate(timestamp,frame){
  ...
  const focus=layers[selected]||layers.electrolyte;partLabel.position.x=THREE.MathUtils.lerp(partLabel.position.x,focus.position.x,.1);selectionBox.visible=anchor.visible;selectionBox.setFromObject(focus);
