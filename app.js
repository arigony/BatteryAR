...
const controller=renderer.xr.getController(0);controller.addEventListener('select',()=>{if(reticle.visible){reticle.matrix.decompose(battery.position,battery.quaternion,battery.scale);battery.scale.setScalar(.42);battery.position.y+=.68;battery.visible=true;placed=true;document.querySelector('#arGuidance').textContent='Bateria posicionada. Caminhe ao redor para observar o modelo.'}});scene.add(controller);
...
document.querySelectorAll('[data-chem]').forEach(b=>b.addEventListener('click',()=>updateChem(b.dataset.chem)));document.querySelectorAll('[data-cycle]').forEach(b=>b.addEventListener('click',()=>updateCycle(b.dataset.cycle)));
